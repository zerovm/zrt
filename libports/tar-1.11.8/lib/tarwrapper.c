/*tarwrapper.c - wrapper arounf tar executable that gets access to 
 *simple archiver interface function.  part of zrt*/
...
#define PREALLLOCATE_TAR_GLOBALS					\
    program_name = "stub";						\
    setlocale (LC_ALL, "");						\
    textdomain (PACKAGE);						\
    ...
    archive_names = 0;							\
    allocated_names = 10;						\
    name_array = (const char **)tar_xmalloc (sizeof (const char *) * allocated_names); \
    names = 0;								\
...
static void tar_parameters_switcher(){
    switch (command_mode)
	{
	case COMMAND_NONE:
	    WARN ((0, 0, _("\
...
int save_as_tar(const char *dir_path, const char *tar_path ){
    ...
    DEBUG_PRINT("param2");
    DEBUG_PRINT(tar_path);
    PREALLLOCATE_TAR_GLOBALS;
    DEBUG_PRINT("2");

    int argc=0;
    char *argv[10];
    argv[argc++] = "stubname"; //#0
    argv[argc++] = "-hcf";     //#1
