...
/* User at first should configure own claster network, and in further can get
 * configuration via interface accessors : ChannelConfFd, ChannelConfPath */
...
/*struct ChannelsConfigInterface should be initialized by this setup function.
It just assign function pointers to ch_if object and initialise it with node id and type*/
void SetupChannelsConfigInterface( struct ChannelsConfigInterface *ch_if, 
				   int ownnodeid, 
				   int ownnodetype );
